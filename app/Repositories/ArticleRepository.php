...
class ArticleRepository
{
    ...
    public function getPromo_dan($promo_name, $num = 1)
    {
        ...
        //select `posts`.*, `feature_posts`.`feature_id` as `pivot_feature_id`, `feature_posts`.`posts_id` as `pivot_posts_id` from `posts` inner join `feature_posts` on `posts`.`id` = `feature_posts`.`posts_id` where `feature_posts`.`feature_id` = 6 and (`published` = 1) and `posts`.`deleted_at` is null order by `order` asc limit 8)
